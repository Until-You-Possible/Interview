...
OK，说完HTTP的基本流程 回到刚才的话题。https解决风险的方式
```
...
```text
摘要算法
摘要算法用来实现完整性。能够为数据生成独一无二的[指纹]。用于校验数据完整性，解决了篡改风险。

```
![img_5.png](./image/img_5.png)
```text
客户端再发送明文之前都会通过摘要算法算出明文的[指纹]，发送的时候把【指纹+明文】一同加密成秘文后，发送给服务器，服务器解密后，用相同的摘要算法
算出发送过来的明文，通过对比客户端携带的【指纹】和当前算出的【指纹】做比较，若【指纹】相同，说明数据是完整的。
...
数字证书
客户端首先向服务端所要公钥，然后用公钥进行加密，服务器收到密文后，用自己的私钥解密。
这个可能会存在问题。如何保证公钥不被篡改和信任度。
这里就要借助第三方权威的结构CA (数字证书认证机构)。将服务器公钥放在数字证书(由数字证书认证机构颁发)中，只要证书是可信的，公钥就是可信的。
如图所示

```
![img_6.png](./image/img_6.png)
通过数字证书的方式保证服务器公钥的身份。解决冒充的问题。
```text
HTTPS是如何建立链接的。期间交互了什么？
SSL/TLS 协议基本流程
- 客户端向服务器所要并验证服务器的公钥
- 双方协商生产【会话密钥】
- 双方采用【会话密钥】进行加密通行

前两步就是ssl/TLS的建立过程。也就是握手阶段。
其实SSL/TLS的【握手阶段】涉及四次通信，如图

```
![img_6.png](./image/900.png)
